...
const CourseDetails = () => {
    ...
    useEffect(() => {
        const getCourseLecture = async () => {
            try {
                const res = await axios.get(`http://localhost:5000/api/v1/course/${courseId}/lecture`, { withCredentials: true })
                if (res.data.success) {
    ...
    }, [courseId]) // Added dependency array

    // Loading state when course is not found
    if (!selectedCourse) {
        return (
            <div className='bg-gray-100 md:p-10 min-h-screen flex items-center justify-center'>
                <div className='text-center'>
                    <h2 className='text-2xl font-bold text-gray-800 mb-4'>Loading Course...</h2>
