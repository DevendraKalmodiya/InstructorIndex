...
const CreateLecture = () => {
    ...
    const createLectureHandler = async ()=>{
        try {
            setLoading(true)
            const res = await axios.post(`http://localhost:5000/api/v1/course/${params?.courseId}/lecture`,{lectureTitle},{
                headers:{
    ...
    useEffect(()=>{
        const getLectures = async()=> {
            try {
                const res = await axios.get(`http://localhost:5000/api/v1/course/${params.courseId}/lecture`,{withCredentials:true})
                if(res.data.success){
